package com.marpies.demo.screens {
    ...
    public class AlertCalloutScreen extends BaseScreen {
        ...
        override protected function initialize():void {
            ...
            alertButton.addEventListener( Event.TRIGGERED, onAlertButtonTriggered );
            ...
            downButton.addEventListener( Event.TRIGGERED, onDownButtonTriggered );
            downButton.layoutData = mTopRightLayoutData;
            addChild( downButton );

            const upButton:Button = new Button();
            upButton.label = "UP";
            upButton.addEventListener( Event.TRIGGERED, onUpButtonTriggered );
            upButton.layoutData = mBottomLeftLayoutData;
            addChild( upButton );

            const leftButton:Button = new Button();
            leftButton.label = "LEFT";
            leftButton.addEventListener( Event.TRIGGERED, onLeftButtonTriggered );
        ...
        private function showCallout( origin:DisplayObject, direction:String ):void {
            Callout.show( feathersLogo, origin, direction ).disposeContent = false;
        }

        private function onRightButtonTriggered( event:Event ):void {
            showCallout( event.currentTarget as DisplayObject, RelativePosition.RIGHT );
        }

        private function onLeftButtonTriggered( event:Event ):void {
            showCallout( event.currentTarget as DisplayObject, RelativePosition.LEFT );
        }

        private function onUpButtonTriggered( event:Event ):void {
            showCallout( event.currentTarget as DisplayObject, RelativePosition.TOP );
        }

        private function onDownButtonTriggered( event:Event ):void {
            showCallout( event.currentTarget as DisplayObject, RelativePosition.BOTTOM );
        }
