package com.marpies.demo.screens {
    ...
    public class MailScreen extends BaseScreen {
        ...
        override protected function initialize():void {
            ...
            mIsTablet = DeviceCapabilities.isTablet( Starling.current.nativeStage );

            layout = (mIsTablet ? (new HorizontalLayoutBuilder()) : new VerticalLayoutBuilder())
                    .setGap( 10 )
                    .build();

            /* Header send button */
            mSendButton = new Button();
            mSendButton.defaultIcon = new Image( MaterialIndigoPinkMobileThemeIcons.sendTexture );
            mSendButton.styleNameList.add( BaseMaterialIndigoPinkMobileTheme.THEME_STYLE_NAME_BUTTON_HEADER_QUIET_ICON_ONLY );
        ...
        private function addInputs():void {
            ...
            group.layoutData = mIsTablet ? new HorizontalLayoutData( 60 ) : new VerticalLayoutData( 100 );
        ...
        private function addMiscControls():void {
            const group:LayoutGroup = new LayoutGroup();
            group.layout = new VerticalLayoutBuilder().setGap( 10 ).setHorizontalAlign( mIsTablet ? HorizontalAlign.RIGHT : HorizontalAlign.LEFT ).build();
            VerticalLayout( group.layout ).paddingTop = 10;
            VerticalLayout( group.layout ).paddingBottom = 10;
            group.layoutData = mIsTablet ? new HorizontalLayoutData( 40 ) : new VerticalLayoutData( 100 );

            /* Attachments button */
            group.addChild( getButton( "Add attachment...", MaterialIndigoPinkMobileThemeIcons.linkTexture ) );

            /* Address book button */
            group.addChild( getButton( "Address book...", MaterialIndigoPinkMobileThemeIcons.peopleTexture ) );
        ...
        private function getButton( label:String, icon:Texture ):Button {
